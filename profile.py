...
for i in range(params.nodeCount):
    ...
    if i == 0:
        node.addService(pg.Execute(shell="bash", command="whoami && sudo chmod +x /local/repository/rootnode.sh && sudo su && /local/repository/rootnode.sh"))
    else:
        node.addService(pg.Execute(shell="bash", command="whoami && sudo chmod +x /local/repository/computenode.sh && sudo su && /local/repository/computenode.sh"))
