...
for i in range(params.nodeCount):
    ...
    if i == 0:
        node.addService(pg.Execute(shell="bash", command="chmod +x /local/repository/rootnode.sh && /local/repository/rootnode.sh"))
    else:
        node.addService(pg.Execute(shell="bash", command="chmod +x /local/repository/rootnode.sh && /local/repository/computenode.sh"))
